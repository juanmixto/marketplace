...
4. **`scripts/agents-status.sh`** — single command that surveys all active worktrees, dirty WIP, stashes (flagging anything >24h per workflow rules), unpushed commits in the shared main repo, listening dev servers, **and any active deploy locks** (#1293 added a `flock` to serialize `npm run deploy:*`). Read it once at session start. If section 6 shows a deploy in progress, **do NOT start your own deploy of the same project** — the second `docker-compose up` will leave Traefik labels orphaned and serve 502s for ~30s. The 2026-05-04 incident was exactly two agents redeploying `marketplaceprod` within minutes.
...
- **Input-validation guards (CI scripts that defend the #1160 hardening)** — `scripts/audit-string-no-max.mjs` (every `z.string()` needs `.max()` or `// allow:unbounded`), `scripts/audit-webhook-cast.mjs` (no `JSON.parse(...) as <Type>` in webhook handlers), `scripts/audit-prisma-update-spread.mjs` (no `data: parsed` / `data: { ...spread }` in `prisma.X.update`/`upsert` — name columns explicitly to stop mass-assignment), `scripts/audit-form-number-coerce.mjs` (no `Number/parseFloat/parseInt(formData.get(...))` — pass the string into a zod schema with `zMoneyEUR` / `z.coerce.number().finite()`). All four ratchet against `*.baseline.json` so net-new violations fail CI without forcing a sweep. Read before adding a server action that writes to DB or a webhook handler.
