...
- **E2E flake debug runbook (4-step playbook, page snapshot first, "3 iterations = wrong layer")** — see [`docs/runbooks/e2e-flake-debug.md`](docs/runbooks/e2e-flake-debug.md). Required reading before patching ANY E2E smoke that "looks flaky". Step 1 is `bash scripts/diagnose-failed-run.sh <pr-number>` — dumps the Playwright page snapshot to stdout. The 2026-05-02 cart-checkout incident wasted ~90 min on three test patches because no one read the snapshot first; turned out to be a real product bug in `CartHydrationProvider` that affected buyers, not a flake.
