 ...
 * confirmation page where the buyer picks the shipping address and
 * first delivery date, reviews the commitment, and only then confirms.
 */
export function SubscribeToBoxButton({ planId, cadence, priceEur, unit }: Props) {
  const cadenceLabel =
    cadence === 'WEEKLY'   ? 'semanal'   :
    cadence === 'BIWEEKLY' ? 'quincenal' :
    'mensual'
  ...
  return (
    <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50/60 p-4 dark:border-emerald-900/50 dark:bg-emerald-950/20">
      ...
      <p className="mt-1 text-xs text-emerald-800 dark:text-emerald-300">
        Suscríbete y recibe una entrega {cadenceLabel} a {formatPrice(priceEur)} / {unit}.
        Podrás saltar entregas, pausar o cancelar cuando quieras.
      </p>
      <Link
        href={`/cuenta/suscripciones/nueva?planId=${planId}`}
