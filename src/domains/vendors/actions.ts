...
export async function updateVendorProfile(input: z.input<typeof profileSchema>) {
  ...
  const ibanWrite = computeIbanColumns(data.iban)
  const bankNameWrite = computeBankNameColumns(data.bankAccountName)

  // #1352 / mass-assignment audit (audit-prisma-update-spread): name
  // every column explicitly so a future schema-loosening cannot leak
  // privileged fields through the form.
