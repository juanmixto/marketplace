...
export type VendorVisualLabelKey =
  | 'vendorVisual.bakery'
  | 'vendorVisual.cheese'
  | 'vendorVisual.winery'
  | 'vendorVisual.orchard'
  | 'vendorVisual.oliveOil'
  | 'vendorVisual.farm'
  | 'vendorVisual.dryland'
  | 'vendorVisual.localProducer'
...
// Source of truth: one row per `VendorCategory` enum value. The `match`
// regex is the fallback heuristic used only when a vendor has no explicit
// category set (legacy rows, seeded vendors). Once an admin — or the
// vendor themselves — picks a category from the GUI, that choice wins.
const VENDOR_VISUAL_RULES: VendorVisualRule[] = [
  ...
  {
    category: 'LOCAL_PRODUCER',
    // Terminal fallback: never matches the regex heuristic, only returned
    // when nothing else does.
    match: /^$a/,
    image: DEFAULT_VENDOR_IMAGE,
    labelKey: 'vendorVisual.localProducer',
  },
]

const RULE_BY_CATEGORY: Record<VendorCategory, VendorVisualRule> = VENDOR_VISUAL_RULES.reduce(
  (acc, rule) => {
    acc[rule.category] = rule
    return acc
  },
  {} as Record<VendorCategory, VendorVisualRule>,
)

export const VENDOR_CATEGORY_VALUES: readonly VendorCategory[] = VENDOR_VISUAL_RULES.map(r => r.category)

export function getVendorCategoryLabelKey(category: VendorCategory): VendorVisualLabelKey {
  return RULE_BY_CATEGORY[category].labelKey
}

function resolveRule(vendor: Pick<VendorVisualInput, 'displayName' | 'description' | 'category'>) {
  if (vendor.category) return RULE_BY_CATEGORY[vendor.category]
  const haystack = `${vendor.displayName} ${vendor.description ?? ''}`
  return VENDOR_VISUAL_RULES.find(rule => rule.match.test(haystack)) ?? RULE_BY_CATEGORY.LOCAL_PRODUCER
}
...
export function getVendorHeroImage(vendor: VendorVisualInput) {
  ...
  return (
    ...
    resolveRule(vendor).image
  )
}

export function getVendorVisualLabelKey(
  vendor: Pick<VendorVisualInput, 'displayName' | 'description' | 'category'>,
): VendorVisualLabelKey {
  return resolveRule(vendor).labelKey
}
