...
import { getVendorIncidentDetail } from '@/domains/incidents/actions'
...
import { VendorIncidentReplyForm } from './VendorIncidentReplyForm'

export async function generateMetadata(): Promise<Metadata> {
  const t = await getServerT()
  return { title: t('vendor.incidents.case') }
}
...
export default async function VendorIncidentDetailPage({ params }: Props) {
  await requireVendor()
  const { id } = await params

  let incident
  try {
    incident = await getVendorIncidentDetail(id)
  } catch (error) {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6 lg:px-8">
      <Link
        href="/vendor/incidencias"
        className="text-sm text-emerald-600 hover:underline dark:text-emerald-400"
      >
        {t('vendor.incidents.detail.backToList')}
      </Link>

      <div className="mt-4 flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <h1 className="text-2xl font-bold text-[var(--foreground)]">
            {t('vendor.incidents.case')} #{incident.id.slice(-6).toUpperCase()}
          </h1>
          <p className="mt-1 text-sm text-[var(--muted)]">
            {t(typeKey)} · {t('vendor.incidents.openedOn')}{' '}
            {incident.createdAt.toLocaleDateString()}
          </p>
          <p className="mt-1 text-xs text-[var(--muted)]">
            {t('vendor.incidents.customer')}: {incident.customer.firstName} {incident.customer.lastName}
            {' · '}
            {t('vendor.incidents.order')} #{incident.order.orderNumber}
          </p>
        </div>
      ...
      <div className="mt-6 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4">
        <p className="text-xs font-semibold uppercase tracking-wide text-[var(--muted)]">
          {t('vendor.incidents.detail.descriptionTitle')}
        </p>
      ...
      <h2 className="mt-8 text-sm font-semibold uppercase tracking-wide text-[var(--muted)]">
        {t('vendor.incidents.detail.thread')}
      </h2>
      ...
      {isClosed ? (
        <p className="mt-6 rounded-lg border border-[var(--border)] bg-[var(--surface-raised)] px-4 py-3 text-sm text-[var(--muted)]">
          {t('vendor.incidents.detail.closed')}
        </p>
      ) : (
        <VendorIncidentReplyForm incidentId={incident.id} />
      )}
    </div>
  )
}
