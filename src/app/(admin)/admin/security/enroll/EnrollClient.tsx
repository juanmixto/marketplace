...
export function EnrollClient() {
  ...
  useEffect(() => {
    ...
    fetch('/api/admin/2fa/enroll', { method: 'POST' })
      ...
      .catch(() => {
        if (!cancelled) setError(t('admin.2fa.startFailed'))
      })
      .finally(() => {
    ...
    // t is stable across renders for a given locale; re-kicking the
    // enrollment POST on every render would rotate the secret.
  ...
  async function handleVerify(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(t('admin.2fa.invalidCode'))
        return
      }
      ...
      setError(t('admin.2fa.verifyError'))
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
        <p className="text-sm font-medium mb-3">{t('admin.2fa.stepScan')}</p>
      ...
      <form onSubmit={handleVerify} className="space-y-3">
        <label className="block">
          ...
          <input
            ...
            placeholder={t('admin.2fa.codePlaceholder')}
            autoFocus
            required
          />
        </label>
        ...
        >
          {submitting ? t('admin.2fa.verifying') : t('admin.2fa.activate')}
        </button>
      </form>
    </div>
  )
