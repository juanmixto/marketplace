...
// Reading cookies via getServerLocale makes this page dynamic. We trade
// the old `revalidate = 3600` (ISR) for per-locale server rendering so the
// home landing no longer ships a 435-LOC client bundle (HomePageClient) to
// every visitor. Data access still goes through getHomeSnapshot, which is
// wrapped in unstable_cache(3600s) — the DB is untouched on a cache hit.
...
const TRUST_ITEMS: { icon: typeof TruckIcon; textKey: TranslationKeys }[] = [
...
function formatStat(s: HomeStat, locale: 'es' | 'en', t: (k: TranslationKeys) => string): string {
  ...
  if (s.kind === 'rating') return `${s.rating.toFixed(1)}★`
  return t(s.valueKey)
}

export default async function HomePage() {
  const [{ featured, categories, vendors, stats }, publicConfig, t, locale] = await Promise.all([
    getHomeSnapshot(),
              ...
              {STEPS.map((s, i) => (
                <div key={s.step} className="relative">
                  ...
                  <div className="relative flex flex-row items-start gap-4 rounded-2xl border border-[var(--border)] bg-[var(--background)] p-4 sm:flex-col sm:gap-0 sm:rounded-none sm:border-0 sm:bg-transparent sm:p-0">
                    <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-emerald-600 text-sm font-bold text-white shadow-md sm:h-12 sm:w-12 sm:rounded-2xl dark:bg-emerald-500 dark:text-gray-950">
                      {s.step}
                    </span>
        ...
        {vendors.length > 0 && (
          <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
            <div className="mb-6 flex flex-col gap-4 rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-widest text-emerald-600 dark:text-emerald-400">
                  {t('sectionLabelOrigin')}
                </p>
                <h2 className="mt-1 text-2xl font-bold text-[var(--foreground)]">
                  {t('sections.featuredVendors')}
                </h2>
                <p className="mt-2 max-w-2xl text-sm text-[var(--muted)]">
                  {t('sections.featuredVendorsDesc')}
                </p>
              </div>
              <Link
                href="/productores"
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {vendors.map(v => {
                ...
                const avgRatingNumber = v.avgRating == null ? null : Number(v.avgRating)
                return (
                  <Link
                    key={v.slug}
                      ...
                      <div className="flex items-start justify-between gap-3">
                        ...
                        {avgRatingNumber !== null && (
                          <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-700 dark:bg-amber-950/30 dark:text-amber-300">
                            <StarIcon className="h-3 w-3" /> {avgRatingNumber.toFixed(1)}
                          </span>
                        )}
                      </div>
                      {v.description && (
                        <p className="mt-3 line-clamp-2 text-sm text-[var(--foreground-soft)]">
                          {v.description}
                        </p>
                      )}
        ...
        <section className="relative overflow-hidden">
          ...
          <div className="relative mx-auto max-w-7xl px-4 py-16 text-center sm:px-6 lg:px-8">
            <p className="mb-3 text-xs font-semibold uppercase tracking-widest text-emerald-400">
              {t('sectionLabelForProducers')}
            </p>
            <h2 className="text-2xl font-bold text-white sm:text-3xl">{t('sections.ctaTitle')}</h2>
            <p className="mx-auto mt-4 max-w-xl text-base leading-relaxed text-emerald-100/80 sm:text-lg">
