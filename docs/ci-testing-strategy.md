...
| Playwright | ✅ `fullyParallel: true` + 2 matrix shards with `--workers=2` inside each shard (4-way effective parallelism). The seeded Postgres is shared per shard; raising workers further without per-worker data isolation (#380) risks cross-test interference. |
