...
async function main() {
  ...
  const lines = [
    ...
    `  byKind         PRODUCT_DRAFT=${aggregates.reviewQueue.byKind.PRODUCT_DRAFT}  VENDOR_DRAFT=${aggregates.reviewQueue.byKind.VENDOR_DRAFT}  DEDUPE_CANDIDATE=${aggregates.reviewQueue.byKind.DEDUPE_CANDIDATE}  UNEXTRACTABLE_PRODUCT=${aggregates.reviewQueue.byKind.UNEXTRACTABLE_PRODUCT}`,
    '',
    'Text reuse (same author + normalised text within window):',
    `  total          ${aggregates.repetition.totalMessages}`,
    `  in-repeat-sets ${aggregates.repetition.messagesInRepeatSets}  (${formatPercent(aggregates.repetition.ratio)})`,
    `  repeat-sets    ${aggregates.repetition.distinctRepeatSets}  (distinct text+author pairs posted ≥2 times)`,
    '',
    'Text length (avg chars, per classification):',
    `  PRODUCT          ${aggregates.textLenByClass.PRODUCT}`,
    `  PRODUCT_NO_PRICE ${aggregates.textLenByClass.PRODUCT_NO_PRICE}`,
    `  CONVERSATION     ${aggregates.textLenByClass.CONVERSATION}`,
    `  SPAM             ${aggregates.textLenByClass.SPAM}`,
    `  OTHER            ${aggregates.textLenByClass.OTHER}`,
    `  productName avg  ${aggregates.drafts.productNameAvgLen}  (draft names; watch for spikes >40)`,
    '',
  ]
