...
const ALLOWLIST_FILES: ReadonlySet<string> = new Set([
  ...
  'src/app/dev/oauth-trigger/page.tsx', // Test-only: under /dev so production proxy 404s it; also gated on MOCK_OAUTH_ENABLED.
  'src/app/dev/oauth-trigger/OAuthTriggerButton.tsx', // Test-only: same gate, button label only visible in Playwright.
